 ...
 * sono quelli di default dati da enum, i.e. numeri interi da 0 a 4: tale
 * fatto verrà sfruttato successivamente usando il valore di currState come
 * indice per accedere agli elementi di array. */
enum state { G, GY, R, OFF, Y } currState;

/* Dichiariamo il tempo di attesa, lo stato dei led e lo stato successivo (sia
 * abilitato che disabilitato) per ognuno degli stati. */
const unsigned wait[] = { 1000, 1000, 1000, 1000, 1000 };
...
const state nextEnabled[] = { GY, R, G, R, R };
const state nextDisabled[] = { OFF, OFF, OFF, Y, OFF };
