...
\begin{itemize}
    \item $\overline{Q_1} \cdot E \to$ led verde acceso $(V)$
    \item $Q_1 \to$ led rosso acceso $(R)$
\end{itemize}

\paragraph{AND-gate per led rosso}
Impiegando una quarta porta AND è possibile rendere l'accensione del led rosso
direttamente dipendente dal segnale di ENABLE (ovvero $R = Q_1 \cdot E$).
Dalla \cref{tab: 2bit} infatti si nota che l'unica differenza di funzionamento
si avrebbe durante la transizione di $E: 1 \to 0$; in questo caso, lo
spegnimento diventerebbe asincrono rispetto al segnale di clock anziché
sincrono.
...
Osserviamo che nel caso $E = 0$, $Q_0 = 1$ e $Q_1 = 0$ che è riportato come
X nella tabella, produce l’output “rosso”. Questo vale a dire che se il
...
\begin{figure}[htbp]
    ...
    Analyzer dei segnali in ingresso e in uscita dall'edge detector di Mealy.
    \label{fig: edgeMoore}}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \caption{Acquisizione di un ciclo completo (frequenza 1 kHz) con Logic
    Analyzer dei segnali in ingresso e in uscita dal detector di Moore.
    \label{fig: edgeMealy}}
\end{figure}
