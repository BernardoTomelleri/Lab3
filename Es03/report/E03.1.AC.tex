...
Una volta inserito il condensatore all'emettitore dalla formula per il
guadagno atteso otteniamo
\[
A_v = -\frac{R_C}{\abs{Z_E}} =
- \frac{R_C}{R_E \; \lvert \rvert \; \left(R\ped{es} + 1/j\omega C_E\right)}
\]

Visto che abbiamo scelto $C_E \gg C\ped{in} \sim C\ped{out}$, alla frequenza
di lavoro $\SI{10}{k\Hz}$ possiamo considerare trascurabile l'impedenza del
condensatore, per cui in buona approssimazione ci aspettiamo
\[
\abs{A_v} =
\frac{R_C}{R_E \; \lvert \rvert \; R\ped{es}} \approx
R_C \abs{\frac{1}{R_E} + \frac{1}{R\ped{es}}}^{-1} =
110 \pm 1
\]
