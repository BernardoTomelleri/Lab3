...
datapath = 'C:/Users/rossi/OneDrive/Desktop/'
f, A1, A2, phi = np.genfromtxt(datapath + 'passabanda.csv', float, delimiter=',', usecols=[0, 1, 2, 3],
                               skip_header=21, unpack=True)
dA2 = np.full(shape=A2.shape, fill_value=8.7*np.sqrt(2*(0.005**2)))
dA1 = np.full(shape=A1.shape, fill_value=8.7*np.sqrt(2*(0.005**2)))
...
plt.annotate("Fb= %.2E \u00B1 %.2E [Hz]" % (
    popt[1], np.sqrt(pcov.diagonal())[1]), (700, -6))
...
print(covar)
print("\n\n\n\n")
f, A1, A2, phi = np.genfromtxt(datapath + 'passabasso.csv', float, delimiter=',', usecols=[0, 1, 2, 3],
                               skip_header=21, unpack=True)
...
plt.plot(f, pb(f, *popt), label="Amplification fit $\chi^2 = %.1f/%d$" %
         (chisq, len(f)-2))
plt.annotate("Fb= %.2E \u00B1 %.2E [Hz]" % (
    popt[0], np.sqrt(pcov.diagonal())[0]), (700, -5))
...
print("\n")
print("\n\n\n")
f, A1, A2, phi = np.genfromtxt(datapath + 'passaalto.csv', float, delimiter=',', usecols=[0, 1, 2, 3],
                               skip_header=21, unpack=True)
dA2 = np.full(shape=A2.shape, fill_value=8.7*np.sqrt(2*(0.005**2)))
dA1 = np.full(shape=A1.shape, fill_value=8.7*np.sqrt(2*(0.005**2)))
...
plt.plot(f, pa(f, *popt), label="Amplification fit $\chi^2 = %.1f/%d$" %
         (chisq, len(f)-2))
plt.annotate("Fa= %.2E \u00B1 %.2E [Hz]" % (
    popt[0], np.sqrt(pcov.diagonal())[0]), (800, -5))
...
popt1, pcov1 = curve_fit(
    retta, f[0:100], A2[0:100], sigma=dA2[0:100], maxfev=7000)
popt2, pcov2 = curve_fit(
    retta, f[250:450], A2[250:450], sigma=dA2[250:450], maxfev=7000)
popt1, pcov1 = curve_fit(
    retta, f[0:100], A2[0:100], sigma=dA2[0:100], maxfev=7000)
...
plt.plot(f[0:300], retta(f[0:300], *popt1), color="black",
         label="Amplification fit $\chi^2 = %.1f/%d$" % (chisq, len(f)-2))
plt.plot(f[0:600], retta(f[0:600], *popt2), color="black",
         label="Amplification fit $\chi^2 = %.1f/%d$" % (chisq, len(f)-2))
...
for i in range(np.size(covar, 0)):
    for j in range(np.size(covar, 1)):
        covar[i][j] /= np.sqrt(pcov1.diagonal())[j] * \
            np.sqrt(pcov1.diagonal())[i]
