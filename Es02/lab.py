...
def pltfitres(model, xmes, ymes, dx=None, dy=None, pars=None, axs=None, in_out=None,
              date=None, space=None):
    """
    Produces standard plot of best-fit curve describing measured data
    with residuals underneath. Space specifies the data range over which to plot model over the data
    ...
    if date is not None:
        ...
        ax1.plot(space, model(space, *pars), **MODEL_CURVE,
                 label=r'fit $\chi^2 = %.1f/%d$' % (chisq, ndof) if np.abs(chisq - ndof) < 2*np.sqrt(ndof)
                 else r'fit $\chi^2 = %.f/%d$' % (chisq, ndof))
