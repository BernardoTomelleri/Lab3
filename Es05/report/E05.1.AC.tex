...
circuito e $410 \pm 5 \; \si{\micro\s}$ nel secondo.
...
Che,come ci aspettavamo risulta, minore della durata dell'impulso trovata
per via della sovrastima sistematica introdotta dal nostro metodo di misura,
ma comunque compatibile entro 2 barre d'errore.
...
$f_L \approx 3 \si{M\Hz}$. Riportiamo di seguito le misure della frequenza
trovate da uno scan con Network per i due circuiti in queste due
configurazioni:
\begin{align*}
f_L = 92.6 \pm 0.1 \; \si{k\Hz}
f_L = 2.96 \pm 0.15 \; \si{M\Hz}
\end{align*}
...
sceltp di misurare direttamente lo slew-rate del TL081
\begin{align*}
...
%Come ci si aspettava, cambiando l'ampiezza dell'onda in ingresso i valori di
%saturazione dell'onda in uscita non risultano cambiare in maniera
%apprezzabile.
...
\begin{align}
T = t_H + t_L = 2 	\tau \ln\left(\frac{1 + q}{1 - q}\right) = 2.2 \pm 0.09 \si{m\s}
